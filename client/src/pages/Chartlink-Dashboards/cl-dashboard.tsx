...
export const CLDashboards: FC = () => {
  const { chartLinkDashboards } = useAppSelector((state) => state.dashboard);
  const [searchTerm, setSearchTerm] = useState('');
  const [renderItems, setRenderItems] = useState(chartLinkDashboards);
  ...
  useEffect(() => {
    const result = searchItems(searchTerm, chartLinkDashboards);
    setRenderItems(result);
  }, [searchTerm]);

  useEffect(() => {
    setSearchTerm('');
    setRenderItems(chartLinkDashboards);
  }, [chartLinkDashboards]);
